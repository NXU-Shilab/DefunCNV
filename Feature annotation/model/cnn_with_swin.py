...
try:
    ...
    kernel_path = os.path.abspath(os.path.join('/mnt/data0/users/baiy/Sei/sei-framework-main/model/SwinTransformer/'))
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=4, shift_size=0,
        ...
        if self.shift_size > 0:
            ...
            attn_mask = attn_mask.masked_fill(attn_mask != 0, float(0.1)).masked_fill(attn_mask == 0, float(0.0))
        else:
            attn_mask = None
...
class Sei_2d_with_Swin(nn.Module):
    def __init__(self, sequence_length=98304, n_genomic_features=552):
        super(Sei_2d_with_Swin, self).__init__()

        self.conv1 = nn.Sequential(
            nn.Conv2d(24, 256, kernel_size=(5, 5), padding=2),
        ...
        self.classifier = nn.Sequential(
            nn.Linear(1000, n_genomic_features),
            nn.ReLU(inplace=True),
            nn.Linear(n_genomic_features, n_genomic_features),
            nn.Sigmoid())

    def forward(self, x):
        x = x.reshape(-1, 4, 768, 128)
        """
        print("x:", x.shape)
        """
        ...
        print("x:",x.shape)
        """
        x = x.reshape(-1, 24, 32, 4, 128)
        """
        print("x:",x.shape)
        """
        x = x.reshape(-1, 24, 128, 128)
        """
        print("x:",x.shape)
        """
        out1 = self.conv1(x)
        ...
        print("spline_out:", spline_out.shape)
        
        reshape_out = spline_out.view(spline_out.size(0), 192 * self._spline_df)
        
        print("reshape_out:", reshape_out.shape)
        """
        out = self.swin_transformer(out)

        """
        print("sout:", out.shape)
        """
